...
extern MPU6050_STRUCT Acc;//MPU6050_STRUCT;mpu6050.Acc.z;MPU6050_STRUCT mpu6050;
//extern float az;
...
//	if(CH_filter[AUX2]<-200&&(MPU6050_STRUCT.Acc.z==8000))	//	if(az>6000&&az<10000)
	if(CH_filter[AUX2]<-200)
	{
		if(ready==0)
			level=0;
		else
		{
			if(mpu6050.Acc.z>6500&&(level==0))
				{
				level++;
			//	if(ultra.height<=60)	
			//	thr=300;
			...
			if(ultra.height<=6)
				...
				if(ultra.height<=30)
									...
									else if(ultra.height>30&&ultra.height<=35)
										thr=390;
